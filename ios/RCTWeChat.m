...
//    callback(@[[WXApi registerApp:appid withDescription:appdesc] ? [NSNull null] : INVOKE_FAILED]);
    callback(@[[WXApi registerApp:appid enableMTA:true] ? [NSNull null] : INVOKE_FAILED]);
}
                     ...
                     callBack:(RCTResponseSenderBlock)callback
{
    ...
    NSLog(@"shareToWeixinWithData type==%@",type);
    if ([type isEqualToString:RCTWXShareTypeText]) {
    ...
    } else if ([type isEqualToString:RCTWXShareTypeMini]){ // 分享小程序
        ...
        NSString * hdImage = aData[@"hdImageData"];
        NSData *hdImageData = [NSData dataWithContentsOfURL:[NSURL URLWithString:hdImage]];
        NSString * withShareTicket = aData[@"withShareTicket"];
        NSInteger  miniProgramType = (NSInteger)aData[@"miniProgramType"];
